                    ...
                    <tr class="border-t-2 border-b-2 bg-white">
                        ...
                        <td>
                            <div class="flex justify-center pl-1 pr-3">
                                <div class="editPendidikan cursor-pointer mr-1" onclick="edit('<?= $row->id_pendidikan ?>', '<?= $row->jenjang ?>', '<?= $row->instansi ?>', '<?= $row->program_studi ?>', '<?= $row->tahun_masuk ?>', '<?= $row->tahun_lulus ?>', '<?= $row->judul_tulisan ?>')"><svg class="lg:w-5 lg:h-5 w-4 h-4" viewBox="0 0 21 20" fill="none" xmlns="http://www.w3.org/2000/svg">
